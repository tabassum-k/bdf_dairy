...
class PartyLedgerSummaryReport:
	...
	def prepare_conditions(self):
		...
		# Handling multi-selected customers
		if self.filters.get("party"):
			customer_list = ', '.join([frappe.db.escape(c) for c in self.filters.get("party")])
			conditions.append(f"party in ({customer_list})")

		if self.filters.party_type == "Customer":
			# Handling multi-selected customer groups
			if self.filters.get("customer_group"):
				group_list = ', '.join([frappe.db.escape(g) for g in self.filters.get("customer_group")])
				conditions.append(
					f"""party in (select name from tabCustomer
					where customer_group in ({group_list}))"""
				)
