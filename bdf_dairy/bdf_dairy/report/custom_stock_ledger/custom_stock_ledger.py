...
def get_columns(filters):
	...
	columns.extend(
		[
			...
			{"label": _("Voucher Entry Type"), "fieldname": "stock_entry_type", "width": 110},
			{
				"label": _("Voucher #"),
...
def get_stock_ledger_entries(filters, items):
    ...
    se = frappe.qb.DocType("Stock Entry")  # Define the Stock Entry doctype

    query = (
        frappe.qb.from_(sle)
        .left_join(se)
        .on(sle.voucher_no == se.name)  # Join Stock Entry on voucher_no
        .select(
            sle.item_code,
            ...
            sle.serial_no,
            sle.project,
            se.stock_entry_type,  # Include stock_entry_type from Stock Entry
        )
        .where(
            (sle.docstatus < 2)
            & (sle.is_cancelled == 0)
